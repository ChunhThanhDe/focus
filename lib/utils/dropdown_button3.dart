...
enum DropdownDirection { textDirection, right, left }
...
typedef SearchMatchFn = bool Function(CustomDropdownMenuItem item, String searchValue);

SearchMatchFn _defaultSearchMatchFn = (item, searchValue) => item.value.toString().toLowerCase().contains(searchValue.toLowerCase());
...
class _DropdownMenuState<T> extends State<_DropdownMenu<T>> {
  ...
  late SearchMatchFn _searchMatchFn;
      ...
      child: CustomPaint(
        ...
        child: Semantics(
          ...
          child: ClipRRect(
            ...
            borderRadius:
                widget.dropdownDecoration?.borderRadius?.resolve(
  ...
  _MenuLimits getMenuLimits(
    ...
    if (preferredMenuHeight > computedMaxHeight) {
      ...
      scrollOffset = math.max(
        0.0,
        selectedItemOffset - (menuHeight / 2) + (itemHeights[selectedIndex] / 2),
      );
...
typedef DropdownButtonBuilder<T> = CustomDropdownMenuItem<T> Function(BuildContext context, T item);
...
class CustomDropdownButton<T> extends StatefulWidget {
  ...
  CustomDropdownButton({
    ...
    // DropdownButtonFormField.
         ...
         customItemsHeights == null || items.isEmpty || customItemsHeights.length == items.length,
         'customItemsHeights list should have the same length of items list',
       ),
       formFieldCallBack = null;

  const CustomDropdownButton._formField({
    super.key,
    required this.items,
    required this.itemBuilder,
    this.childBuilder,
  ...
  final DropdownButtonBuilder<T> itemBuilder;
  ...
  final Widget Function(BuildContext context, T? item, VoidCallback onTap)? childBuilder;
...
class CustomDropdownButtonState<T> extends State<CustomDropdownButton<T>> with WidgetsBindingObserver {
  ...
  void _updateSelectedIndex() {
    ...
    if (widget.items.isEmpty || value == null || !widget.items.contains(widget.value)) {
      _selectedIndex = null;
      return;
    }
  ...
  Rect _getRect() {
    ...
    final NavigatorState navigator = Navigator.of(
      context,
      rootNavigator: widget.dropdownFullScreen,
    );
  ...
  void _handleTap() {
    ...
    final List<_MenuItem<T>> menuItems = <_MenuItem<T>>[
      for (int index = 0; index < widget.items.length; index += 1)
        _MenuItem<T>(
          item: widget.itemBuilder(context, widget.items[index]),
          onLayout: (Size size) {
  ...
  double get _denseButtonHeight {
    ...
    final double scaledFontSize = textScaler.scale(fontSize);
    return math.max(
      scaledFontSize,
        ...
        child: Builder(
          builder: (context) {
            if (_selectedIndex != null) {
              return (widget.selectedItemBuilder ?? widget.itemBuilder)(
                    context,
    ...
    return Semantics(
      ...
      child: Actions(
        ...
        child: InkWell(
          ...
          borderRadius: widget.buttonDecoration?.borderRadius?.resolve(
            Directionality.of(context),
          ),
          child: result,
        ),
...
class CustomDropdownButtonFormField<T> extends FormField<T> {
  ...
  CustomDropdownButtonFormField({
    ...
    required DropdownButtonBuilder<T> itemBuilder,
...
class _DropdownButtonFormFieldState<T> extends FormFieldState<T> {
  ...
  void didChange(T? value) {
    ...
    final CustomDropdownButtonFormField<T> dropdownButtonFormField = widget as CustomDropdownButtonFormField<T>;
...
class CustomDropdownMenuItem<T> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Hoverable(
      builder:
          (context, hovering, child) => DefaultTextStyle(
            style: DefaultTextStyle.of(context).style.copyWith(
              color: hovering && hoverTextColor != null ? hoverTextColor : textColor,
              fontSize: 13,
            ),
            child: child!,
          ),
